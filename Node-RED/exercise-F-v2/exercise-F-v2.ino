...
void setup() {
  ...
  doc["calibracion"] = "Calibrando sensor PIR....";
  serializeJson(doc, Serial);
  Serial.println();

  pinMode(PINPIR, INPUT); //Inicializar el PIR
  delay(60000);//Calibración del PIR
  doc["calibracion"] = "Fin de calibración PIR";
  serializeJson(doc, Serial);
  Serial.println();
...
void enviarJSON(){
    if(tiempoActual - tiempoInicialJSON >= delayJSON){ // Se le resta el tiempo actual cuando el delay sea mayor o igual a 3000 (delay JSON)
      ...
      sistema_monitoreo["latitud"] = 5.619454609357036;
      sistema_monitoreo["longitud"] = -73.81699785629826;
